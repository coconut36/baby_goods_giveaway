class MessagesController < ApplicationController
  before_action :current_user_must_be_message_recipient,
  ...
  def index
    @q = current_user.messages_received.ransack(params[:q])
  ...
  def current_user_must_be_message_recipient
    set_message
    unless current_user == @message.recipient
  ...
  def message_params
    params.require(:message).permit(:sender_id, :text, :product_id,
                                    :recipient_id)
  end
end
